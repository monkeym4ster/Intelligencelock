...
public class ExcelExportSXXSSF {
	 ...
	 * @return
	 */
	public static ExcelExportSXXSSF start(String filePath, String fileWebPath,
			String filePrefix, List<String> fieldNames,
	...
	public String exportFile() throws Exception {
		String filename = filePrefix + "_" + MyUtil.getCurrentTimeStr()
				+ ".xlsx";

		// 文件保存路径
		File fileDir = new File(filePath + fileWebPath);
		// 目录不存在&&是一个目录
		if (!fileDir.exists()) {
			fileDir.mkdirs();
		}

		FileOutputStream out = new FileOutputStream(filePath + fileWebPath
				+ filename);
		wb.write(out);
		out.flush();
		out.close();
		...
		return filePath + fileWebPath + filename;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		String filePath = "F:/upload/linshi/";
		// 导出文件的前缀
		String filePrefix = "authorizataion";
